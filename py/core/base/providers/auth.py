...
class AuthProvider(Provider, ABC):
    ...
    def auth_wrapper(
        self,
        public: bool = False,
        is_internal_api_call: bool = False,
    ):
        # --- RAGAI CUSTOMIZATION: Check for trusted header first ---
        async def _auth_wrapper_for_internal_api_call(request: Request) ->  RagAiAuthContext:
            """
            Auth logic for trusted internal calls. Validates trusted headers and returns
            a full context object including the R2R user and other RagAI headers.
            """
            # 1. Get all trusted headers from the request
            authenticated_user_id = request.headers.get("x-authenticated-user-id", None)
            customer_api_key = request.headers.get("x-authenticated-client-key", None)
            target_model_id = request.headers.get("x-target-model", None)
            target_rag_config_id = request.headers.get("x-target-rag-config", None)

            # 2. Validate that the essential headers are present
            if not authenticated_user_id:
                logger.warning("RAGAI: Internal auth called, but one or more trusted headers are missing.")
                raise R2RException(
                    message="Forbidden: Missing required RagAI context headers.",
                    status_code=403,
                )

            # 3. Fetch the R2R user from the database using the trusted ID
            logger.info(f"RAGAI: Found trusted user ID header: {authenticated_user_id}. Fetching user from R2R DB.")
            user = await self.database_provider.users_handler.get_user_by_id(authenticated_user_id)
            
            if not (user and user.is_active):
                logger.error(f"RAGAI: User ID '{authenticated_user_id}' from trusted header not found or inactive in R2R DB.")
                raise R2RException(message="Invalid user context.", status_code=403)

            logger.info(f"RAGAI: Authenticated as R2R user '{user.email}' via trusted header.")
            
            return RagAiAuthContext(
                user=user,
                api_key=customer_api_key,
                target_model=target_model_id,
                target_rag_config=target_rag_config_id
            )
             
        async def _auth_wrapper_for_external_api_call(
        ...
        return _auth_wrapper_for_external_api_call if not is_internal_api_call else _auth_wrapper_for_internal_api_call
