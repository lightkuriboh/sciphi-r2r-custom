...
class BaseRouterV3:
    ...
    def set_rate_limiting(self):
        ...
        async def get_external_user(
            user: User = Depends(self.providers.auth.auth_wrapper())
        ) -> User:
            return user

        async def get_internal_api_call_user_from_context(
            auth_context: RagAiAuthContext = Depends(
                self.providers.auth.auth_wrapper(is_internal_api_call=True)
            ),
        ) -> User:
            return auth_context.user

        def RateLimiterAuthFactory(user_provider_func) -> Depends:
            """
            Factory that combines a user provider with the core rate-limiting logic.
            It creates a single dependency that resolves the user and then runs the checks.

            Args:
                user_provider_func: The actual function to get a user (e.g., get_external_user),
                                    NOT Depends(get_external_user).
            """
            async def full_rate_limiter(
                request: Request,
                auth_user: User = Depends(user_provider_func)
            ):
                """
            ...
            return full_rate_limiter

        self.rate_limit_dependency = RateLimiterAuthFactory(
            user_provider_func=get_external_user
        )

        self.rate_limit_dependency_for_internal_call = RateLimiterAuthFactory(
            user_provider_func=get_internal_api_call_user_from_context
        )
